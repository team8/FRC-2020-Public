...
//        TODO: make super accurate (can only be at most 2 inches off)
        ...
        DepotToRocketShip.add(new Waypoint(new Translation2d(-(kHabLineX + PhysicalConstants.kRobotLengthInches + kOffsetX), 0), 100)); //goes straight at the start so the robot doesn't get messed up over the ramp
...
//        routines.add(new TimeoutRoutine(1)); //placeholder
    ...
    public Routine takeHatch() { //rocket ship close to loading station
        ...
        CargoShipToLoadingStation.add(new Waypoint(new Translation2d(-(kHabLineX - PhysicalConstants.kRobotLengthInches * .5 + kOffsetX), -(kLeftLoadingStationY - PhysicalConstants.kRobotWidthInches * .4 + kOffsetY)), SPEED)); //lines up with loading station
...
//        routines.add(new TimeoutRoutine(1)); //placeholder
    ...
    public Routine placeHatchFar() { //loading station to rocket ship close
        ...
        backLoadingStationToRocketShip.add(new Waypoint(new Translation2d(-(kLeftRocketShipMidX + kOffsetX), -(findLineFar(kLeftRocketShipMidX + PhysicalConstants.kRobotLengthInches * 1.8) + kOffsetY)), SPEED)); //goes around the rocket ship
        ...
        forwardLoadingStationToRocketShip.add(new Waypoint(new Translation2d(-(kLeftRocketShipFarX + PhysicalConstants.kRobotLengthInches * 1.6 + kOffsetX), -(findLineFar(kLeftRocketShipMidX + PhysicalConstants.kRobotLengthInches * 1.6) - PhysicalConstants.kRobotLengthInches * .3 + kOffsetY)), SPEED)); //line up with rocket ship far
...
//        routines.add(new TimeoutRoutine(1)); //placeholder
    ...
    public Routine takeCargo() { //rocket ship close to depot - could be more accurate
        ...
        RocketShipToDepot.add(new Waypoint(new Translation2d(-(kCargoShipLeftFrontX + PhysicalConstants.kRobotLengthInches + kOffsetX), -(kLeftDepotY + PhysicalConstants.kRobotLengthInches * .2 + kOffsetY)), 180)); //turn around the rocket ship
...
//        routines.add(new TimeoutRoutine(1)); //placeholder
    ...
    public Routine placeCargoMid() { //depot to rocket ship mid
        ...
        DepotToRocketShipMid.add(new Waypoint(new Translation2d(-(kLeftDepotX + PhysicalConstants.kRobotLengthInches * 2 + kOffsetX), -(kLeftDepotY + kOffsetY)), SPEED)); //go straight until near rocket ship
        DepotToRocketShipMid.add(new Waypoint(new Translation2d(-(kLeftRocketShipMidX - PhysicalConstants.kRobotLengthInches * .3), -(kLeftDepotY + kOffsetY)), SPEED));
        DepotToRocketShipMid.add(new Waypoint(new Translation2d(-(kLeftRocketShipMidX), -(kLeftRocketShipMidY - PhysicalConstants.kRobotLengthInches * .8)), SPEED)); //line up with rocket ship
