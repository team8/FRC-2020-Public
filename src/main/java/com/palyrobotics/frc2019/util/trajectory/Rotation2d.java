...
public class Rotation2d implements Interpolable<Rotation2d> {
    ...
    /**
     * From trig, we know that sin^2 + cos^2 == 1, but as we do math on this object we might accumulate rounding errors. Normalizing forces us to re-scale the
    ...
    public Rotation2d rotateBy(Rotation2d other) {
        return new Rotation2d(cosAngle * other.cosAngle - sinAngle * other.sinAngle, cosAngle * other.sinAngle + sinAngle * other.cosAngle,
                true);
    }
    ...
    public Rotation2d interpolate(Rotation2d other, double x) {
        ...
        double angleDelta = inverse().rotateBy(other).getRadians();
        return rotateBy(Rotation2d.fromRadians(angleDelta * x));
    }

    @Override
    public String toString() {
        var format = new DecimalFormat("#0.000");
        return String.format("(%s degrees)", format.format(getDegrees()));
    }
}