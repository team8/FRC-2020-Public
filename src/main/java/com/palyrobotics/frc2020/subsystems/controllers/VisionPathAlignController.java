...
public class VisionPathAlignController extends DriveRamseteController {
	...
	public void updateSignal(@ReadOnly Commands commands, @ReadOnly RobotState state) {
		...
		if (mCounter++ > mConfig.trajectoryUpdateCycle || mFirstTimeRun) {
			...
			Log.debug("drive pose" + new Pose2d(Units.inchesToMeters(mLimelight.getPnPTranslationX()),
					Units.inchesToMeters(mLimelight.getPnPTranslationY()),
					Rotation2d.fromDegrees(mLimelight.getYawToTarget())).toString());
			Log.debug("goal pose" + RobotState.getInstance().drivePose.getTranslation().toString());
